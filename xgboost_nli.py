...
from clean_data import encode_sentence, label_mapping
...
def x_and_y_from_data(
    df: pd.DataFrame, encoder, tokenizer
    ...
    X = np.array(
        [
            encode_sentence(encoder, tokenizer, f"{x} [SEP] {y}", DEVICE)
...
if __name__ == "__main__":
    for dataset in ["match", "mismatch"]:
        training_file: str = "Data/SICK/train_cleaned_ph.csv"
        validation_file: str = f"Data/SICK/valid_cleaned_ph.csv"
        testing_file: str = f"Data/SICK/test_cleaned_ph.csv"
        ...
        X_train, y_train = x_and_y_from_data(
            df=train_df, encoder=bbu_model, tokenizer=bbu_tokenizer
        )
        print(X_train.shape, y_train.shape)
        print("Training loaded")
        X_val, y_val = x_and_y_from_data(
            df=valid_df, encoder=bbu_model, tokenizer=bbu_tokenizer
        )
        print(X_val.shape, y_val.shape)
        print("Validation loaded")
        X_test, y_test = x_and_y_from_data(
            df=test_df, encoder=bbu_model, tokenizer=bbu_tokenizer
        )
        print(X_test.shape)
        ...
        if training_file.split("/")[1] == "MultiNLI":
            ...
            output_df = label_mapping(
                output_df, "gold_label", "gold_label", str_to_int=False
            )
